...
st.set_page_config(page_title="NanoScope", layout="wide", page_icon="🔬")
st.title("🔬 NanoScope")
...
def origin_style_ax(ax):
    ax.grid(False)
    ax.tick_params(direction="in", length=5, width=1.2, labelsize=11, top=False, right=False)
    ax.minorticks_on()
...
def plot_iv_line(V, I, title="I–V Curve", y_in_nA=True, show_fit=False, fit_G=None):
    ...
    y = I * 1e9 if y_in_nA else I
    ax.plot(V, y, color="black", linewidth=1.8, label="IV data")
    ax.set_ylabel("Current (nA)" if y_in_nA else "Current (A)", fontsize=12)

    if show_fit and fit_G is not None:
        yfit = (fit_G * V) * 1e9 if y_in_nA else fit_G * V
        ax.plot(V, yfit, color="blue", linewidth=1.8, label="Fit")
...
def plot_kde_lines(datasets, labels, title, xlabel, ylabel="Density", bandwidth=1.0):
    valid = [(np.asarray(d), lab) for d, lab in zip(datasets, labels) if len(d) > 1]
    if not valid:
        st.warning("Not enough data points to plot smooth line curves.")
        return

    global_min = min(np.min(d) for d, _ in valid)
    global_max = max(np.max(d) for d, _ in valid)
    x_grid = np.linspace(global_min, global_max, 1000)

    fig, ax = plt.subplots(figsize=(5.5, 4.2), dpi=300)

    colors = ["black", "blue", "gray", "purple", "green", "orange"]
    for i, (data, label) in enumerate(valid):
        kde = gaussian_kde(data, bw_method=bandwidth)
        style = "-" if "Total" in label else "--"
        lw = 2.0 if "Total" in label else 1.7
        ax.plot(x_grid, kde(x_grid), linestyle=style, linewidth=lw, color=colors[i % len(colors)], label=label)
...
def circle_overlap_area(R, r, x):
    ...
    term3 = 0.5 * np.sqrt((-x + r + R) * (x + r - R) * (x - r + R) * (x + r + R))
...
def pick_linear_region_auto(V, I, eps=0.005, window=0.05, min_points=6):
    ...
    while True:
        ...
        V_lin = V[mask]
        I_lin = I[mask]
        if V_lin.size >= min_points and np.sum(V_lin**2) > 0:
...
def mc_cyl_diameter(G_nS, dG_nS, sigma, dsigma, L_nm, dL_nm, include_access=True, N=100000, seed=7):
    ...
    return (
        float(np.mean(d_nm)),
        float(np.std(d_nm, ddof=1)),
        tuple(map(float, np.percentile(d_nm, [2.5, 97.5]))),
        int(d_nm.size),
    )
...
def compute_conical_radius(V, I, n, K, L, theta, method, eps=0.005, window=0.05, min_points=6):
    if method == "auto_window_brentq":
        ...
        intercept = 0.0
        meta = {"fit_method": "through-origin near-zero window", "used_window": used_window, "points_used": len(V_lin)}
        ...
        meta = {"fit_method": "polyfit with intercept", "used_window": None, "points_used": int(np.sum(mask))}
    else:
...
CONICAL_STAGE_CONFIG = {
    "1 mM NaCl (auto-window, brentq)": {"label": "1 mM NaCl", "K": K_MAP["1 mM NaCl"], "method": "auto_window_brentq"},
    "10 mM NaCl (auto-window, brentq)": {"label": "10 mM NaCl", "K": K_MAP["10 mM NaCl"], "method": "auto_window_brentq"},
    "100 mM NaCl (auto-window, brentq)": {"label": "100 mM NaCl", "K": K_MAP["100 mM NaCl"], "method": "auto_window_brentq"},
    "1 M NaCl (polyfit + intercept, fsolve)": {"label": "1 M NaCl", "K": K_MAP["1 M NaCl"], "method": "polyfit_fsolve"},
    "After functionalization / antibody (10 mM, auto-window, brentq)": {"label": "10 mM NaCl", "K": K_MAP["10 mM NaCl"], "method": "auto_window_brentq"},
    "Antibody/biosensing IV (10 mM, polyfit + intercept, fsolve)": {"label": "10 mM NaCl", "K": K_MAP["10 mM NaCl"], "method": "polyfit_fsolve"},
}
...
# Navigation
# ============================================================
page = st.sidebar.radio("Navigation", ["Home", "Size Calculator", "ΔI Range Explorer"])
...
Predict possible current blockade values for biomolecules entering the nanopore with different orientations.
'''
    )
...
if page == "Size Calculator":
    ...
    if up is not None:
        ...
        if name.endswith(".txt"):
            ...
            st.success("Loaded TXT → converted to clean CSV columns.")
            st.download_button("Download cleaned CSV", df_iv.to_csv(index=False).encode("utf-8"), "IV_clean.csv", "text/csv")
    ...
    if mode.startswith("CBD"):
        ...
        if use_iv:
            if df_iv is None:
                ...
                if V is not None:
                    ...
                    plot_iv_line(V, I, "I–V Curve with Global Fit", y_in_nA=plot_nA, show_fit=True, fit_G=G_S)
                    st.info(f"Global slope through origin: **G = {G_from_iv_nS:.2f} nS**")
        ...
        include_access = st.checkbox("Include access resistance", value=True)
        N = st.selectbox("Monte Carlo samples", [20000, 50000, 100000, 200000], index=2)

        if st.button("Calculate CBD diameter"):
            mean, std, (lo, hi), n_ok = mc_cyl_diameter(G_nS, dG_nS, sigma, dsigma, L_nm, dL_nm, include_access, N)
            if np.isnan(mean):
                st.error("Could not compute. Check inputs.")
            else:
        ...
        stage = st.selectbox("Which conical scenario?", list(CONICAL_STAGE_CONFIG.keys()))
        ...
        if "1 mM" in stage:
            st.warning("At 1 mM, double-layer effects may make this an apparent radius.")
        ...
        if df_iv is None:
            ...
            if V is not None:
                ok = np.isfinite(V) & np.isfinite(I)
                V, I = V[ok], I[ok]
                plot_iv_line(V, I, "I–V Curve", y_in_nA=plot_nA)
                ...
                if st.button("Compute conical radius"):
                    try:
                        result = compute_conical_radius(V, I, n, K, L, theta, method, eps=float(eps), window=float(window), min_points=int(min_points))
                        st.write(f"Fit method: {result['meta']['fit_method']}")
                        if result["meta"]["used_window"] is not None:
                ...
                compare_eps = st.number_input("Comparison eps", value=0.005, step=0.001, format="%.4f", key="compare_eps")
                compare_window = st.number_input("Comparison start window", value=0.05, step=0.01, format="%.3f", key="compare_window")
                compare_min_points = st.number_input("Comparison min points", value=6, step=1, key="compare_min_points")
                ...
                if st.button("Compare radii across 1 mM / 10 mM / 100 mM / 1 M"):
                    ...
                    for label, K_val in K_MAP.items():
                        try:
                            if label == "1 M NaCl":
                                res = compute_conical_radius(V, I, n, K_val, L, theta, "polyfit_fsolve", eps=max(float(compare_eps), 0.05))
                            else:
                                res = compute_conical_radius(V, I, n, K_val, L, theta, "auto_window_brentq", eps=float(compare_eps), window=float(compare_window), min_points=int(compare_min_points))
...
if page == "ΔI Range Explorer":
    ...
    with col2:
        ...
        occupancy = st.slider("Occupancy factor", 0.3, 1.0, 1.0, 0.05)
        st.caption("Occupancy scales effective blocker size.")
    ...
    if np.isfinite(d_m):
        ...
        st.error("Could not infer pore diameter. Check inputs.")
    ...
    if model.startswith("Sphere"):
        ...
        if st.button("Compute ΔI (sphere)"):
            di = delta_i(i0_A, d_m, L_m, V, sigma, dbio_nm * 1e-9 * occupancy)
            if np.isfinite(di):
        ...
        event_mode = st.selectbox(
            "Event model",
            ["Centered translocation", "Bump / partial entry", "Adsorption / rim interaction", "Combined mixture"]
        )
        ...
        if event_mode == "Combined mixture":
            ...
            st.caption(f"Normalized weights: centered={w_centered:.2f}, bump={w_bump:.2f}, adsorption={w_ads:.2f}")

        add_noise = st.checkbox("Add Gaussian measurement noise", value=True)
        noise_pA = st.slider("Noise SD (pA)", 0.0, 1000.0, 20.0, 1.0)
        curve_source = st.selectbox("Curve source", ["Noisy prediction", "Theoretical prediction", "Both"])
        ...
        if st.button("Compute ΔI range (ellipsoid)"):
            rng = np.random.default_rng(seed)
            a, b, c = (A_nm / 2) * 1e-9, (B_nm / 2) * 1e-9, (C_nm / 2) * 1e-9
            pore_radius = d_m / 2.0

            nvec = random_unit_vectors(N, rng)
            Aproj = projected_area_ellipsoid(a, b, c, nvec)
            rbio_eff = np.sqrt(Aproj / np.pi)

            di_centered, di_bump, di_ads = [], [], []
            offset_list_nm, blocked_area_list_nm2, event_labels = [], [], []
            ...
            for r_eff in rbio_eff:
                ...
                if event_type == "Centered translocation":
                    ...
                    offset = rng.uniform(max(0.0, pore_radius - 0.3 * r_eff), pore_radius + 0.8 * r_eff)
                else:
                    offset = rng.uniform(max(0.0, pore_radius - 0.8 * r_eff), pore_radius + 0.2 * r_eff)
            ...
            di_theory_all = np.concatenate([x for x in [di_centered, di_bump, di_ads] if x.size])

            if add_noise and noise_pA > 0:
                di_centered_noisy = di_centered + rng.normal(0.0, noise_pA, len(di_centered)) if di_centered.size else np.array([])
                di_bump_noisy = di_bump + rng.normal(0.0, noise_pA, len(di_bump)) if di_bump.size else np.array([])
                di_ads_noisy = di_ads + rng.normal(0.0, noise_pA, len(di_ads)) if di_ads.size else np.array([])
            else:
                di_centered_noisy, di_bump_noisy, di_ads_noisy = di_centered.copy(), di_bump.copy(), di_ads.copy()

            di_noisy_all = np.concatenate([x for x in [di_centered_noisy, di_bump_noisy, di_ads_noisy] if x.size])

            di_for_stats = di_theory_all if curve_source == "Theoretical prediction" else di_noisy_all
            ...
            if stats is None:
                ...
                datasets, labels = [], []
                if curve_source in ["Theoretical prediction", "Both"]:
                    datasets.append(di_theory_all)
                    labels.append("Total theoretical fit")
                if curve_source in ["Noisy prediction", "Both"]:
                    datasets.append(di_noisy_all)
                    labels.append("Total noisy fit")

                if show_component_curves:
                    if curve_source == "Theoretical prediction":
                        comp_map = [("Centered component", di_centered), ("Bump component", di_bump), ("Adsorption component", di_ads)]
                    else:
                        comp_map = [("Centered component", di_centered_noisy), ("Bump component", di_bump_noisy), ("Adsorption component", di_ads_noisy)]

                    for label, data in comp_map:
                ...
                plot_kde_lines(
                    datasets,
                    labels,
                    title="Predicted ΔI Distribution",
                    xlabel="ΔI (pA)",
                    ylabel="Density",
                ...
                if event_mode == "Combined mixture":
                    st.dataframe(pd.DataFrame([
                        {"Component": "Centered", "Count": len(di_centered), "Median ΔI (pA)": np.median(di_centered) if len(di_centered) else np.nan},
                        {"Component": "Bump", "Count": len(di_bump), "Median ΔI (pA)": np.median(di_bump) if len(di_bump) else np.nan},
                        {"Component": "Adsorption", "Count": len(di_ads), "Median ΔI (pA)": np.median(di_ads) if len(di_ads) else np.nan},
        ...
        if st.button("Compute ΔI range (rod)"):
            ...
            if stats is None:
                ...
                fig, ax = plt.subplots(figsize=(5.5, 4.2), dpi=300)
                ax.plot(np.degrees(theta), di_pA, color="black", linewidth=1.8)
